...
do
  ...
  if [[ -f "$INPUT" ]]; then
    ...
    then
      echo "  -> ${INPUT} ..."
      BLANK_CHECK_OUTPUT=$(magick convert "${INPUT}" -shave ${DPI}x${DPI} -density $(expr $DPI/2) -adaptive-resize 65% -virtual-pixel White -blur 0x15 -fuzz 15% -trim info: 2>/dev/null)
